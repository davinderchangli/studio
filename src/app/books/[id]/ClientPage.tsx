...
export default function ClientPage({ book }: ClientPageProps) {
            ...
            <div className="space-y-6">
              ...
              {book.sizes.length > 1 && (
                <div className="space-y-4">
                  ...
                  <RadioGroup
                    value={selectedSize}
                    onValueChange={(value: BookSize) => setSelectedSize(value)}
                    className="flex gap-4"
                  >
                    {book.sizes.map((size) => (
                      <div key={size} className="flex items-center space-x-2">
                        <RadioGroupItem value={size} id={size} />
                        <Label htmlFor={size} className="cursor-pointer text-base">
                          {size}
                        </Label>
                      </div>
                    ))}
